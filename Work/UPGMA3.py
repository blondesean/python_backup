...
Made for URC Model Off Round 1

Author: Sean Duncan
Date: August 2015
Purpose: 
  -Cluster people from a random sample into profiles
  -Repeat this for 25 Samples
  -Prepare data for K-means clustering into occupation groups
'''

#Library Declareations 
import numpy as np
import pandas as pd
import array
import random
...
#Import libraries check
print('\n')
print('Got Libraries!')
print('\n')

#Time the length of execution
...
#set file paths
training = '\\\\chnas01\\urc\\URC-Private\\Research\\Modeling Team\\Users\\Sean\\Misc\\Experimental Modeling\\Bootstrap Samples\\Occ_Groups_10k'
figurelocation = '\\\\chnas01\\urc\\URC-Private\\Research\\Modeling Team\\Users\\Sean\\Misc\\Experimental Modeling\\Processed Figures\\linkage_clusters'
processeddata = '\\\\chnas01\\urc\\URC-Private\\Research\\Modeling Team\\Users\\Sean\\Misc\\Experimental Modeling\\Processed Samples\\Occ_Groups_10k_processed'

#How many bootstraps are there?
K = 25

#Start at what cluster?
N = 9

#loop to cluster the data sets 
LoopTime = [] #store value of how long each loop took
while N <= K:

  #What round are we on?
  print('Starting Round '+ str(N) )
  loop_start = time.time() #time each loop
  print('\n') 

  #Fix the strings based on what round it is 
  training2 = training + '_' + str(N) + '.csv'
  figurelocation2 = figurelocation + '_' + str(N) + '.png'
  processeddata2 = processeddata + '_' + str(N) + '.xlsx'

  #read the CSV into a dataframe
  df = pd.read_csv(training2, header = 0, nrows = 10000) # nrows = 10000
  print('Data read!')
  print('\n')

  #subset to only the columns that we care about
  #Chars that were used in testing 
  '''
  'Education'
  ,'Age_G'
  ,'gender'
  ...
  #Save the output, save figure as average_clusters
  plt.savefig(figurelocation2, dpi=200) 
  print('Dendrogram Saved!')
  print('\n')

  #create the output data set for profile analysis 
  df.to_excel(processeddata2)
  print('Clustering Data Saved!')
  print('\n')

  #Ending for the loop
  print('---Done---')
  loop_end = time.time() #get the end time for the loop
  LoopTime.append(round(loop_end - loop_start,1)) #seconds each loop took
  print('\n')

  #Go into the next round of clustering 
  N = N + 1
...
i = 0
while (i <= K - 1):
  print('Bootstrap Sample', i + 1, 'took', LoopTime[i], 'seconds to cluster.')
  i = i + 1
print('\n')
...
print('The code started running at', start_time + '.', '\n' \
      + 'The code ended at', end_time + '.', '\n' \
      + 'The code took', round(end_time_n - start_time_n, 1), 'seconds to run.')
print('\n')
